<?php
     ...
     */
    class dbMan extends \mysqli implements iSingleton
    {
         ...
         * @param string $dbName
         */
        public function __construct($host = DBHOST, $user = DBUSER, $pass = DBPWD, $dbName = DBNAME)
        {
...
//            }
         ...
         * @return DBMan
         */
        static public function get_instance($host = DBHOST, $user = DBUSER, $pass = DBPWD, $dbName = DBNAME, $isNewInstance = false)
        {
            if ($isNewInstance)
            {
                $tmp = new dbMan($host, $user, $pass, $dbName);
                $tmp->renewConnection($host, $user, $pass, $dbName);
                return $tmp;
            }
         ...
         */
        static public function destroySingleton()
        {
            if(isset(self::$_instance))
            {
        ...
        {
            $update = true;
            if (isset($this->_connectionInfo))
            {
                if ($this->_connectionInfo['host'] !== $host || $this->_connectionInfo['user'] !== $user)
                {
                    $this->real_connect($host, $user, $pass, $dbName);
                    $update = true;
                }
                else if ($this->_connectionInfo['dbName'] !== $dbName)
                {
                    $update = true;
                    ...
                    {
                        $update = false;
                        $this->select_db($this->_connectionInfo['dbname']);
                    }
                }
                else
                {
         ...
         */
        public function query($query, array $params = array(), $reconnect = true)
        {
            $q = $this->safeQuery($query, $params);
                ...
                if ($this->errno === 2006 && $reconnect)
                {
                    if (isset($this->_connectionInfo))
                    {
                        $this->real_connect(
                            ...
                            $this->_connectionInfo["pass"],
                            $this->_connectionInfo["dbName"]
                        );
                        return $this->query($query, $params, false);
                    }
                }
                Utils::logError("mysqli_error", "query error", "query : ".$q." | error : ".$this->error);
            }
            ...
            $callback = function($matches) use (&$paramIndex, &$params, &$helper)
            {
                ...
                if (preg_match('/%(v[s|d|f])/i', $result, $matchedParam) === 1 && is_array($params[$paramIndex]))
                {
                    ...
                    switch ($matchedParam[0])
                    {
                        case 'vs':
                        {
                            $typeCast = function(&$param) { $param = strval($param); };
                            break;
                        }
                        case 'vd':
                        {
                            $typeCast = function(&$param) { $param = intval($param); };
                            break;
                        }
                        case 'vf':
                        {
                            $typeCast = function(&$param) { $param = doubleval($param); };
                            break;
                        }
                    }
        ...
        private static function escapeCallback(&$value, $key, DBMan $helper)
        {
            if (is_array($value))
            {
                array_walk($value, 'dbMan::escapeCallback', $helper);
                $value = implode("','", $value);
            }
            else
        ...
        # Compatibility layer with the old SqlManager
        public function query2assoc($query, array $params = array()) { return $this->multiResQuery($query, $params); }
        public function query2list($query, array $params = array()) { return $this->singleResQuery($query, $params, MYSQLI_NUM); }
        public function singleQuery($query, array $params = array(), $mode = MYSQLI_ASSOC) { return $this->singleResQuery($query, $params, $mode); }
        public function last_insert_id() { return $this->insert_id; }
        public function affected_rows() { return $this->affected_rows; }
        public function getSafeQuery($query, array $params = array()) { return $this->safeQuery($query, $params); }

        /**
         * Recursive function escaping all data inside an array
         * @param $array
         * @return mixed
         */
        public function escapeArray($array)
        {
            ...
            foreach($array as $key=>$row)
            {
                if (is_array($row))
                {
                    $escapedArray[$key] = $this->escapeArray($row);
                }
                else
                {
                    $escapedArray[$key] = $this->real_escape_string($row);
                }
            }
            return $escapedArray;
        }
    ...
    class dbResult implements \Iterator, \Countable
    {
        ...
        public function fetch_array($resulttype = MYSQLI_BOTH, $xss = false)
        {
            ...
            {
                ++$this->_recordNumber;
                if ($xss) array_walk($res, 'dbResult::xssProtectCallback');
            }
        ...
        public function fetch_fields($xss = false)
        {
            ...
            if ($xss && $res) array_walk($res, 'dbResult::xssProtectCallback');
            return $res;
        }
        ...
        public function fetch_all($resulttype = MYSQLI_ASSOC, $xss = false)
        {
            ...
            if ($xss && $res) array_walk_recursive($res, 'dbResult::xssProtectCallback');
            return $res;
        }
